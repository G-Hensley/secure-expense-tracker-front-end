    ...
    return (
        <>
            <div className="flex flex-col gap-4 py-6 px-4 items-center justify-between expense-card w-full">
                <div className={`flex gap-4 sm:flex-none justify-between w-full ${isEditing && 'flex-col sm:flex-row'}`}>
                    {isEditing ? (
                ...
                <div className="flex gap-4 w-fit">
                    ...
                    <button aria-label="Delete" className='font-mono self-center w-fit cursor-pointer bg-white text-black text-md px-2 py-1 rounded-lg hover:opacity-60 
                        transition-all duration-300 active:scale-95' onClick={() => handleDeleteExpense(id)}>DELETE</button>
                    </div>
            </div>
        </>
    )
